= Knative connector aws-sns-sink
...
This connector uses the https://camel.apache.org/camel-kamelets/aws-sns-sink.html[aws-sns-sink] Kamelet and consumes events from the Knative broker.
...
metadata:
  ...
  labels:
    eventing.knative.dev/connector: aws-sns-sink
...
spec:
  template:
    spec:
      containers:
        - image: quay.io/openshift-knative/aws-sns-sink:1.0
          name: log
          env:
