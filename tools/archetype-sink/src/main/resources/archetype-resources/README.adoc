= Knative connector ${kamelet-name}-sink
...
This connector uses the https://camel.apache.org/camel-kamelets/${kamelet-name}-sink.html[${kamelet-name}-sink] Kamelet and consumes events from the Knative broker.
...
spec:
  template:
    spec:
      containers:
        - image: quay.io/openshift-knative/${kamelet-name}-sink:1.0
          name: log
          env:
