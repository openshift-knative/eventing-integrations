= Knative connector aws-ddb-streams-source
...
kind: Pipe
metadata:
  name: kn-connector-aws-ddb-streams-source
spec:
  source:
    ref:
      apiVersion: camel.apache.org/v1
...
This connector uses the https://camel.apache.org/camel-kamelets/aws-ddb-streams-source.html[aws-ddb-streams-source] Kamelet that produces events for the Knative broker.
...
spec:
  template:
    spec:
      containers:
        - image: quay.io/openshift-knative/kn-connector-aws-ddb-streams-source:1.0
